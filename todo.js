...
function finishToDo(event) {
  // checkbox 이벤트를 받아서 부모를 찾아서 class name을 넣어준다.
...
function deleteToDo(event) {
  ...
  window.localStorage.removeItem(li.id); // 저장된 checkbox 상태 지우기
  const cleanToDos = toDos.filter(function (toDo) {
    //필터를 이용해 todoo.id 와 li.id가 다르면 살리고 같으면 버린다.
...
function paintToDo(text) {
  //localStorage에 저장된 text 및 이벤트 입력란으로 들어오는 값을 할일란에 표시한다.
...
function handleSubmit(event) {
  // 할일란에 입력되는 event를 받아서 할일란에 쓴다. (paintToDo 함수 실행)
...
function loadToDos() {
  //localStorage에 저장된 데이터가 있다면 text값을 개별로 불러와서 할일란에 쓴다.(paintToDo 함수 실행)
